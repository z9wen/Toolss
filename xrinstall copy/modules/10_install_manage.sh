        ...
        if [[ -n $(ls /opt/xray-agent/subscribe_local/default/) ]]; then
            ...
            find /opt/xray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -z "${showStatus}" ]]; then
                    ...
                    # sing-box
                    if [[ -f "/opt/xray-agent/subscribe_local/sing-box/${email}" ]]; then
                        cp "/opt/xray-agent/subscribe_local/sing-box/${email}" "/opt/xray-agent/subscribe/sing-box_profiles/${emailMd5}"

                        echoContent skyBlue " ---> 下载 sing-box 通用配置文件"
                        if [[ "${release}" == "alpine" ]]; then
                            wget -O "/opt/xray-agent/subscribe/sing-box/${emailMd5}" -q "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/documents/sing-box.json"
                        else
                            wget -O "/opt/xray-agent/subscribe/sing-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/documents/sing-box.json"
                        fi

                        jq ".outbounds=$(jq ".outbounds|map(if has(\"outbounds\") then .outbounds += $(jq ".|map(.tag)" "/opt/xray-agent/subscribe_local/sing-box/${email}") else . end)" "/opt/xray-agent/subscribe/sing-box/${emailMd5}")" "/opt/xray-agent/subscribe/sing-box/${emailMd5}" >"/opt/xray-agent/subscribe/sing-box/${emailMd5}_tmp" && mv "/opt/xray-agent/subscribe/sing-box/${emailMd5}_tmp" "/opt/xray-agent/subscribe/sing-box/${emailMd5}"
                        jq ".outbounds += $(jq '.' "/opt/xray-agent/subscribe_local/sing-box/${email}")" "/opt/xray-agent/subscribe/sing-box/${emailMd5}" >"/opt/xray-agent/subscribe/sing-box/${emailMd5}_tmp" && mv "/opt/xray-agent/subscribe/sing-box/${emailMd5}_tmp" "/opt/xray-agent/subscribe/sing-box/${emailMd5}"
...
updateRemoteSubscribe() {
    ...
    while read -r line; do
        ...
        if ! echo "${singBoxSubscribe}" | grep -q "nginx" && [[ -n "${singBoxSubscribe}" ]]; then
            ...
            singBoxSubscribe=$(jq ". +=${singBoxSubscribe}" "/opt/xray-agent/subscribe_local/sing-box/${email}")
            echo "${singBoxSubscribe}" | jq . >"/opt/xray-agent/subscribe_local/sing-box/${email}"
...
initRealityKey() {
    ...
    if [[ -z "${realityPrivateKey}" ]]; then
        if [[ "${selectCoreType}" == "2" || "${coreInstallType}" == "2" ]]; then
            echoContent green "自动生成密钥对...\n"
            realityX25519Key=$(/opt/xray-agent/sing-box/sing-box generate reality-keypair)
            realityPrivateKey=$(echo "${realityX25519Key}" | head -1 | awk '{print $2}')
            realityPublicKey=$(echo "${realityX25519Key}" | tail -n 1 | awk '{print $2}')
            echo "publicKey:${realityPublicKey}" >/opt/xray-agent/sing-box/conf/config/reality_key
        else
