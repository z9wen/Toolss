...
readInstallType() {
    ...
    if [[ -d "/opt/xray-agent" ]]; then
        if [[ -f "/opt/xray-agent/xray/xray" ]]; then
            # 检测xray-core
            if [[ -d "/opt/xray-agent/xray/conf" ]] && [[ -f "/opt/xray-agent/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/opt/xray-agent/xray/conf/02_trojan_TCP_inbounds.json" || -f "/opt/xray-agent/xray/conf/07_VLESS_vision_reality_inbounds.json" ]]; then
                # xray-core
                ...
                if [[ -f "/opt/xray-agent/sing-box/sing-box" ]] && [[ -f "/opt/xray-agent/sing-box/conf/config/06_hysteria2_inbounds.json" || -f "/opt/xray-agent/sing-box/conf/config/09_tuic_inbounds.json" || -f "/opt/xray-agent/sing-box/conf/config/20_socks5_inbounds.json" ]]; then
                    singBoxConfigPath=/opt/xray-agent/sing-box/conf/config/
                fi
            fi
        elif [[ -f "/opt/xray-agent/sing-box/sing-box" && -f "/opt/xray-agent/sing-box/conf/config.json" ]]; then
            # 检测sing-box
            ctlPath=/opt/xray-agent/sing-box/sing-box
            coreInstallType=2
            configPath=/opt/xray-agent/sing-box/conf/config/
            singBoxConfigPath=/opt/xray-agent/sing-box/conf/config/
        fi
    fi
}
