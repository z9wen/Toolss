...
unInstallSingBox() {
    local type=$1
    if [[ -n "${singBoxConfigPath}" ]]; then
        if grep -q 'tuic' </opt/xray-agent/sing-box/conf/config.json && [[ "${type}" == "tuic" ]]; then
            rm "${singBoxConfigPath}09_tuic_inbounds.json"
            echoContent green " ---> 删除sing-box tuic配置成功"
        fi

        if grep -q 'hysteria2' </opt/xray-agent/sing-box/conf/config.json && [[ "${type}" == "hysteria2" ]]; then
...
cleanUp() {
    if [[ "$1" == "xrayDel" ]]; then
        ...
        handleSingBox stop
        rm -rf /opt/xray-agent/sing-box/conf/config.json >/dev/null 2>&1
        rm -rf /opt/xray-agent/sing-box/conf/config/* >/dev/null 2>&1
    fi
}
